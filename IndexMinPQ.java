...
public class IndexMinPQ<Key extends Comparable<Key>> implements Iterable<Integer> {
    ...
    public void insert(int i, Key key) {
        ...
        if(n + 1 == maxN)
        {
            ...
            Key[] keys_ = (Key[]) new Comparable[maxN + 1];    // make this of length maxN??
            int[] pq_   = new int[maxN + 1];
            int[] qp_   = new int[maxN + 1];                   // make this of length maxN??
            ...
            for(int w = qp.length; w < qp_.length; w++)
            {
                qp_[w] = -1;
            }
            keys = keys_;
